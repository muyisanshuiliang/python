...
class Solution:
    def accountsMerge(self, accounts: List[List[str]]) -> List[List[str]]:
        result = []
        email_dict = {}
        for item_index, item_val in enumerate(accounts):
            index = None
            for item_email_inx in range(1, len(item_val)):
                index = email_dict.get(item_val[item_email_inx], None)
                if index != None:
                    break
            if index != None:
                cur_user = result[index]
                for item_email_inx in range(1, len(item_val)):
                    if not cur_user.__contains__(item_val[item_email_inx]):
                        cur_user.insert(1, item_val[item_email_inx])
                        email_dict[item_val[item_email_inx]] = index
            else:
                cur_user = []
                cur_user.insert(0, item_val[0])
                for item_email_inx in range(1, len(item_val)):
                    cur_user.insert(1, item_val[item_email_inx])
                    email_dict[item_val[item_email_inx]] = len(result)
